...
@Component({
  selector: 'app-dialogreconexiones',
  templateUrl: './dialogreconexiones.component.html',
  styleUrls: ['./dialogreconexiones.component.css']
})
export class DialogreconexionesComponent implements OnInit {
  ...
  loadNovedades() {
    this._novedad.getReconexiones(this.token).subscribe(res => {
      ...
      if (res.data.length) {
        ...
        this.toastWarning("No se encuentran registros de reconexiones para el mes en curso");
        this.listaNovedades = [];
        this.dataSource = new MatTableDataSource(this.listaNovedades);
        this.dataSource.paginator = this.paginator;
      }
  ...
  cargarFecha() {
    if (this.fechaDesde == '' || this.fechaHasta == '') {
      ...
      this._novedad.getFechaReconexion(this.fechaDesde, this.fechaHasta, this.token).subscribe(res => {
        ...
        if (res.data.length) {
          ...
          this.toastWarning("No se encuentran registro de reconexiones entre las fechas " + this.fechaDesde + ' ' + this.fechaHasta);
          this.listaNovedades = [];
          this.dataSource = new MatTableDataSource(this.listaNovedades);
          this.dataSource.paginator = this.paginator;
        }
